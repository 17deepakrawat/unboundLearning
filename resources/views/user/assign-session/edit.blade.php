...
@section('page-script')
  <script type="module">
    $(function() {
      ...
      $("#admission_session_id").val({{ $userSharing->admission_session_id }}).trigger('change');
      $('#admission_session_id option').each(function() {
        if ($(this).val() !== "{{ $userSharing->admission_session_id }}") {
          $(this).prop('disabled', true);
        }
      });
      $("#start_date").val(moment('{{ $userSharing->start_date }}').format("DD-MM-YYYY")).trigger('change');
  ...
  <script>
    function getSchemes() {
      ...
      var selectedSchemes = {!! json_encode($userSharingFees->pluck('scheme_id')->toArray()) !!}
      var checkedIds = [];
      $.ajax({
        ...
        success: function(response) {
          if (response.status) {
            ...
            $.each(response.data, function(key, value) {
              var checked = '';
              if (selectedSchemes.includes(value.id)) {
                checked = 'checked="checked"';
                checkedIds.push(value.id);
              }
              schemeDom += '<div class="col-md-12 mb-2">' +
                '<div class="form-check mt-2">' +
                '<input class="form-check-input" type="checkbox" name="scheme_ids[]" ' + checked + ' onchange="getFeeStructures(this.value)" value="' + value.id + '" id="scheme-check-' + value.id + '">' +
            ...
            $.each(checkedIds, function(index, value) {
              $("#scheme-check-" + value).trigger('change');
            })
          }
        }
      })
    ...
    function getFeeStructures(schemeId) {
      if ($("#scheme-check-" + schemeId).prop("checked") == true) {
        var selectedFeeStructureIds = {!! json_encode($userSharingFees->pluck('fee_structure_id')->toArray()) !!}
        $.ajax({
            ...
            if (response.status) {
              ...
              $.each(response.data, function(key, value) {
                if (value.has_sharing || !value.is_constant) {
                  ...
                  var isChecked = value.has_sharing ? 'checked onclick="return false;"' : selectedFeeStructureIds.includes(value.id) ? 'checked' : '';
    ...
    function getFeeSharingTypes(schemeId) {
      ...
      var selectedFeeSharingType = {!! json_encode($userSharingFees->pluck('fee_type')->toArray()) !!}

      var typeDom = '';
      $.each(types, function(key, value) {
        var isChecked = selectedFeeSharingType.includes(value) ? 'checked' : '';
        typeDom +=
          '<div class="form-check mt-2 me-2">' +
          '<input class="form-check-input" type="checkbox" name="fee_sharing_type[' + schemeId + ']" onchange="getFeeSharing(' + schemeId + ')" value="' + value + '" ' + isChecked + ' id="fee-sharing-type-check-' + schemeId + key + '">' +
    ...
    function getFee(schemeId, feeStructureId, duration = 0, specializationId = 0) {
      var fees = {!! json_encode($userSharingFees) !!}
      const feeData = fees.find(fee =>
        fee.scheme_id === schemeId &&
        fee.fee_structure_id === feeStructureId &&
        (duration === 0 || fee.duration === duration) &&
        (specializationId === 0 || fee.specialization_id === specializationId)
      );
      return feeData ? feeData.fee : null;
    }
        ...
        success: function(response) {
          ...
          if (selectedFeeType == undefined) {
            ...
            $.each(feeStructureIds, function(key, value) {
              ...
              var feeValue = getFee(parseInt(schemeId), feeStructure.id);
              var inputField = '<input type="number" class="form-control border-none" min="0" max="' + maxValue + '" value="' + feeValue + '" name="fee[' + schemeId + '][' + feeStructure.id + ']" placeholder="' + placeholder + '">'
            ...
            for (var i = 1; i <= maxDuration; i++) {
              ...
              $.each(feeStructureIds, function(key, value) {
                ...
                var maxValue = feeStructure.has_sharing ? "100" : "",
                  placeholder = feeStructure.has_sharing ? "Sharing " + verticalSharingOn : "Variable Amount";
                var feeValue = getFee(parseInt(schemeId), feeStructure.id, i);
                var inputField = '<input type="number" class="form-control border-none" min="0" max="' + maxValue + '" value="' + feeValue + '" name="fee[' + schemeId + '][' + feeStructure.id + '][' + i + ']" placeholder="' + placeholder + '">'
            ...
            $.each(specializations, function(key, value) {
              ...
              $.each(feeStructureIds, function(key, feeStructureId) {
                ...
                var maxValue = feeStructure.has_sharing ? "100" : "",
                  placeholder = feeStructure.has_sharing ? "Sharing " + verticalSharingOn : "Variable Amount";
                var feeValue = getFee(parseInt(schemeId), feeStructure.id, 0, value.id);
                var inputField = '<input type="number" class="form-control border-none" min="0" max="' + maxValue + '" value="' + feeValue + '" name="fee[' + schemeId + '][' + feeStructure.id + '][' + value.id + ']" placeholder="' + placeholder + '">'
                tbody += '<td>' + inputField + '</td>';
              })
              tbody += '</tr>';
            })
            ...
            $.each(specializations, function(key, value) {
              for (var i = 1; i <= maxDuration; i++) {
                ...
                $.each(feeStructureIds, function(key, feeStructureId) {
                  ...
                  var maxValue = feeStructure.has_sharing ? "100" : "",
                    placeholder = feeStructure.has_sharing ? "Sharing " + verticalSharingOn : "Variable Amount";
                  var feeValue = getFee(parseInt(schemeId), feeStructure.id, i, value.id);
                  var inputField = '<input type="number" class="form-control border-none" min="0" max="' + maxValue + '" value="' + feeValue + '" name="fee[' + schemeId + '][' + feeStructure.id + '][' + value.id + '][' + i + ']" placeholder="' + placeholder + '">'
                  tbody += '<td>' + inputField + '</td>';
                })
                tbody += '</tr>';
              }
  ...
  <script type="module">
    ...
    $("#assignSessionForm").submit(function(e) {
      ...
      if ($("#assignSessionForm").valid()) {
        ...
        formData.append("id", "{{ $userSharing->id }}")
