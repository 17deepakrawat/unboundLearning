...
<script>
  ...
  function getUserDownlines(userId, callback) {
    $.ajax({
      url: '/users/downlines/' + userId,
      type: 'GET',
      dataType: 'json',
      success: function(response) {
        if (response.status == 'success') {
  ...
  function getUsers(verticalId, parentId = 0) {
    if ($("#vertical_" + verticalId).prop("checked") == true) {
      ...
      getUserDownlines(userId, function(downlines) {
        $.ajax({
          ...
          success: function(response) {
            if (response.status == 'success') {
              $.each(response.data, function(key, value) {
                if (value.id != parseInt(userId)) {
                  if(!downlines.includes(value.id)){
                    $('#user_' + verticalId).append('<option value="' + value.id + '">' + value.name + ' (' + value.email + ')</option>');
                  }
                }
              });
