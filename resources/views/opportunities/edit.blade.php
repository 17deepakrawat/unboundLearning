...
<form id="opportunityEditForm" action="{{ route('manage.opportunity.update', [$opportunity->id]) }}" method="POST">
  @method('PUT')
  <div class="modal-body">
    <div class="row g-3">
      <div class="col-md-12">
        <label class="form-label" for="name">Full Name</label>
        <input type="text" id="name" name="name" class="form-control" placeholder="ex: Full Name" autofocus value="{{ $opportunity->name }}" />
      </div>
      <div class="col-md-6">
        <label class="form-label" for="email">Email</label>
        <input type="email" id="email" name="email" class="form-control" placeholder="ex: mail@example.com" value="{{ $opportunity->email }}">
      </div>
      <div class="col-md-6">
        <label class="form-label" for="phone">Mobile</label>
        <input type="tel" id="phone" name="phone" class="form-control phone" placeholder="ex: 987654XXX" value="{{ $opportunity->phone }}">
      </div>
      <div class="col-md-6">
        <label class="form-label" for="date_of_birth">DOB</label>
        <input type="tel" id="date_of_birth" name="date_of_birth" class="form-control" placeholder="ex: DD-MM-YYYY" value="{{ !empty($opportunity->lead->date_of_birth) ? date('d-m-Y', strtotime($opportunity->lead->date_of_birth)) : '' }}">
      </div>
      <div class="col-md-6">
        ...
        <select class="form-control select2" id="vertical_id" name="vertical_id" onchange="getPrograms()">
          ...
          @foreach ($verticals as $vertical)
            <option value="{{ $vertical->id }}" {{ $opportunity->vertical->id == $vertical->id ? 'selected' : '' }}>{{ $vertical->fullName }}</option>
          @endforeach
        </select>
      </div>
      <div class="col-md-6">
...
<script>
  $(function() {
    ...
    const dialingCode = "{{ $opportunity->country_code ? $opportunity->country_code : '+91' }}";
    ...
    $("#opportunityEditForm").submit(function(e) {
      e.preventDefault();
      if ($("#opportunityEditForm").valid()) {
  ...
  $(".select2").select2({
    placeholder: 'Choose',
    dropdownParent: $('#opportunityEditForm')
  });
  ...
  function getPrograms() {
    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        setTimeout(() => {
          $("#program_id").val("{{ $opportunity->program_id }}").trigger('change');
        }, 100);
      }
    })
  }
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.status == 'success') {
          ...
          setTimeout(() => {
            $("#specialization_id").val("{{ $opportunity->specialization_id }}").trigger('change');
          }, 100);
        }
      }
    })
