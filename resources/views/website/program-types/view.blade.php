@php
  ...
  $content = !empty($programType->content) ? json_decode($programType->content, true) : [];
...
{{-- Meta Section --}}
@section('title')
  {{ array_key_exists('title', $tags) ? $tags['title'] : $programType->name }}
@endsection
    ...
    <div id="landingHero" class="section-py landing-hero position-relative">
      ...
      <div class="container">
        <div class="hero-text-box text-center">
          <h1 class="text-primary hero-title display-6 fw-bold">{{ $programType->name }}</h1>
        </div>
      </div>
  </section>
        ...
        <div class="col-md-12">
          <div class="card shadow-none">
            <div class="card-body">
              <div class="row">
                <div class="col-md-8">
                  <h4 class="fw-bold">{{ $programType->name }}</h4>
                  <p class="lh-4">
            ...
            <div class="row">
              ...
              <div class="col-md-8" style="padding: 30px">
                <h4 class="text-center mb-4">
                  ...
                  <span class="position-relative fw-bold z-1"> benefits of an {{ $programType->name }}
  ...
  <section class="section-py">
    <div class="container">
      <h4 class="mb-4 fw-bold">
        Institutes and Boards offering {{ $programType->name }}
      </h4>
      <div class="row gy-3">
        @foreach ($programType->departmentVerticals as $departmentVertical)
          <div class="col-6 col-md-3">
            <div class="card border border-label-primary shadow-none">
              <div class="card-body text-center">
                <a href="/institutions-and-boards/{{ $departmentVertical->vertical->slug }}">
                  <img src="{{ asset($departmentVertical->vertical->logo) }}" width="auto" height="85px" alt="{{ $departmentVertical->vertical->name . ' (' . $departmentVertical->vertical->vertical_name . ')' }}" class="ratio ratio-16x9 mb-3" />
                  <h6 class="fw-medium text-truncate fw-bold mb-0">
                    {{ $departmentVertical->vertical->short_name . ' (' . $departmentVertical->vertical->vertical_name . ')' }}
                  </h6>
                  <small class="fs-12 text-muted">{{ $departmentVertical->department->name }}</small>
                </a>
              </div>
            </div>
          </div>
                      ...
                      <div class="card mb-3">
                        <div class="card-img-top">
                          <div class="col-md-12 text-center rounded-top bg-body p-4">
                            <h5 class="mt-3" style="font-size: 16px; font-weight:800">{{ $program->name }}</h5>
                          </div>
                        </div>
              ...
              <div class="col-md-3">
                <div class="card">
                  <a href="/departments/{{ $department->slug }}">
                    <div class="card-body text-center">
                      @if (!empty($images['icons']['icon']))
                        <i class="{{ $images['icons']['icon'] }} ti-xl"></i>
                      @else
                        <img src="{{ $images['icons']['image'] }}" height="64px">
                      @endif
                      <h6 class="fw-bold mb-0 text-nowrap">{{ $department->name }}</h6>
                    </div>
                  </a>
                </div>
              </div>
        ...
        <div class="col-md-12 col-lg-12 col-xl-12">
          <div class="swiper-reviews-carousel overflow-hidden mb-5 pb-md-2 pb-md-3">
            <div class="swiper" id="swiper-courses">
              <div class="swiper-wrapper">
                @foreach ($otherProgramTypes as $key => $programType)
                  <div class="swiper-slide">
                    <div class="card mb-3">
                      <div class="card-img-top">
                        <div class="col-md-12 text-center rounded-top bg-body-secondary p-4">
                          <h5 class="mt-3" style="font-size: 14px; font-weight:800">{{ $programType->name }}</h5>
                        </div>
                      </div>
                      <a href="/program-types/{{ $programType->slug }}">
                        <div class="card-body">
                          <h5 class="card-title fs-13 mb-0">{{ $programType->name }}</h5>
                          <p class="card-text">
                            <small class="fs-11 fw-bold">{{ $programType->programs->count() }} Programs</small>
                          </p>
                        </div>
                      </a>
                    </div>
