...
@section('page-script')
  ...
  <script type="module" >
    $(function() {
      var canAdd = "{{ Auth::user()->hasPermissionTo('create career-testimonial') ? true : false }}";
      const addButton = canAdd ? {
        text: 'Add Team Talk',
        className: 'add-new btn btn-primary mb-3 mb-md-0 waves-effect waves-light',
        attr: {
          'onclick': "add('{{ route('website.content.career.testimonial.create') }}', 'modal-lg')",
        },
        init: function(api, node, config) {
          $(node).removeClass('btn-secondary');
        }
      } : '';
      var canEdit = "{{ Auth::user()->hasPermissionTo('edit career-testimonial') ? true : false }}";
      var dataTable = $('#testimonial-table').DataTable({
        ajax: "{{route('website.content.career.testimonial')}}",
        columns: [
          {
            data: 'name'
          },
        ...
        columnDefs: [
          {
            ...
            render: function(data, type, full, meta) {
              var dropDown = '<span type="button" data-bs-toggle="dropdown" aria-expanded="false">' +
                ...
                '<li><a class="dropdown-item" href="javascript:void(0);" onclick="edit(&#39;/website/content/career/testimonial/edit/' + full['id'] + '&#39;, &#39;modal-lg&#39;)">Edit</a></li>'
                '</ul>';
              return dropDown;
            },
            // visible: canEdit
