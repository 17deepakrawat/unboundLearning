...
@php
    $filterJson = session()->has('applicationfilterjson')?session()->get('applicationfilterjson'):[];
...
<form id="applicationFilterAddForm" action="{{ route('manage.application.filter') }}" method="POST">
...
<script>
    $(document).ready(function() {
        $("#applicationFilterAddForm").submit(function(e) {
        e.preventDefault();
        if (formValidate()) {
      ...
      $('.addNewFilterRow').on('click', function() {
        $.ajax({
          url: "/settings/application/assignmets/filter",
          type: 'get',
    ...
    $('.validation').on('change', function() {
      ...
      if (value == 'In' || value == 'Not In') {
        ...
        $(div).select2({
          dropdownParent:$('#applicationFilterAddForm')
        })
      } else {
    ...
    {   
      $.ajax({
        url:"/manage/application/filter/reset",
        type:'get',
        success:function(res)
        {
          $(".modal").modal('hide');
