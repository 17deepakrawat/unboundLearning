            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        ...
                        $('#application_owner_id').val("{{$opportunity['application_owner_id']}}");
                        $('#application_owner_id').trigger('change');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        ...
                        $('#admission_type_id').val("{{$opportunity['admission_type_id']}}");
                        $('#admission_type_id').trigger('change');
                    }
                }
            })
        });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            $('#specialization_id').val("{{$opportunity['specialization_id']}}");
        ...
        $('#specialization_id').on('change', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#admission_duration').val("{{$opportunity['admission_duration']}}");
                    $('#admission_duration').trigger('change');
                }
            })
        })
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 'success') {
                        ...
                        $('#country_id').val("{{$opportunityCustomFields['country_id']}}");
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == 'success') {
                            ...
                            $('#state_id').val("{{$opportunityCustomFields['state_id']}}");
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == 'success') {
                            ...
                            $('#city_id').val("{{$opportunityCustomFields['city_id']}}");
                                ...
                                @foreach ($step->fields as $field)
                                    <div class="col-md-4">
                                        <label class="form-label"
                                            for="{{ $field->customFields->schema }}">{{ $field->customFields->name }}</label>
                                        @if ($field->customFields->type == 'Dropdown')
                                            <select
                                                class="form-select {{ $field->customFields->is_core_field == 0 ? 'custom-dropdowns' : '' }}"
                                                id="{{ $field->customFields->schema }}"
                                                name="{{ $field->customFields->schema }}{{ $field->customFields->is_multiple ? '[]' : '' }}"
                                                {{ $field->customFields->mandatory ? 'required' : '' }}
                                                {{ $field->customFields->is_multiple ? 'multiple' : '' }}
                                                {!! $field->customFields->child
                                                    ? 'onchange="getDependentOptions(&#39;' .
                                                        $field->customFields->schema .
                                                        '&#39;,' .
                                                        $field->customFields->child->id .
                                                        ')"'
                                                    : '' !!}>
                                                ...
                                                @if ($field->customFields->is_core_field == 0)
                                                    @php
                                                        ...
                                                        if (!empty($field->customFields->pre_selected_options)) {
                                                            $decodedOptions = json_decode(
                                                                $field->customFields->pre_selected_options,
                                                                true,
                                                            );
                                                    ...
                                                    @foreach (json_decode($field->customFields->options, true) as $option)
                                                        <option value="{{ $option }}"
                                                            {{ in_array($option, $preSelectedOptions) ? 'selected' : '' }}>
                                                            {{ $option }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        @elseif ($field->customFields->type == 'Dependent Dropdown')
                                            <select class="form-select dependent-dropdowns"
                                                id="{{ $field->customFields->schema }}"
                                                name="{{ $field->customFields->schema }}{{ $field->customFields->is_multiple ? '[]' : '' }}"
                                                {{ $field->customFields->mandatory ? 'required' : '' }}
                                                {{ $field->customFields->is_multiple ? 'multiple' : '' }}>
                                        ...
                                        @elseif($field->customFields->type == 'Text')
                                            <input type="text" class="form-control"
                                                id="{{ $field->customFields->schema }}"
                                                value="{{ !empty($opportunity) ? (array_key_exists($field->customFields->schema, $opportunity) ? $opportunity[$field->customFields->schema] : (array_key_exists('lead', $opportunity) && array_key_exists($field->customFields->schema, $opportunity['lead']) ? $opportunity['lead'][$field->customFields->schema] : (!empty($opportunityCustomFields->toArray())&&array_key_exists($field->customFields->schema, $opportunityCustomFields->toArray())?$opportunityCustomFields[$field->customFields->schema]:''))) : '' }}"
                                                name="{{ $field->customFields->schema }}"
                                                {{ $field->customFields->mandatory ? 'required' : '' }}>
                                        ...
                                        @elseif($field->customFields->type == 'Phone')
                                            <input type="tel"
                                                class="form-control {{ $field->customFields->is_intl_phone == 1 ? 'make_international' : '' }}"
                                                id="{{ $field->customFields->schema }}"
                                                value="{{ !empty($opportunity) ? (array_key_exists($field->customFields->schema, $opportunity) ? $opportunity[$field->customFields->schema] : (array_key_exists('lead', $opportunity) && array_key_exists($field->customFields->schema, $opportunity['lead']) ? $opportunity['lead'][$field->customFields->schema] : (!empty($opportunityCustomFields->toArray())&&array_key_exists($field->customFields->schema, $opportunityCustomFields->toArray())?$opportunityCustomFields[$field->customFields->schema]:''))) : '' }}"
                                                name="{{ $field->customFields->schema }}"
                                                {{ $field->customFields->mandatory ? 'required' : '' }}>
                                        @elseif($field->customFields->type == 'Date')
                                            <input type="text" class="form-control datepicker"
                                                id="{{ $field->customFields->schema }}" placeholder="dd-mm-yyyy"
                                                value="{{ !empty($opportunity) ? (array_key_exists($field->customFields->schema, $opportunity) ? $opportunity[$field->customFields->schema] : (array_key_exists('lead', $opportunity) && array_key_exists($field->customFields->schema, $opportunity['lead']) ? $opportunity['lead'][$field->customFields->schema] : (!empty($opportunityCustomFields->toArray())&&array_key_exists($field->customFields->schema, $opportunityCustomFields->toArray())?$opportunityCustomFields[$field->customFields->schema]:''))) : ''  }}"
                                                name="{{ $field->customFields->schema }}"
                                                {{ $field->customFields->mandatory ? 'required' : '' }}>
                                        @elseif($field->customFields->type == 'Textarea')
                                            <textarea class="form-control" rows="1" id="{{ $field->customFields->schema }}"
                                                placeholder="{{ $field->customFields->name }}" name="{{ $field->customFields->schema }}"
                                                {{ $field->customFields->mandatory ? 'required' : '' }}>{{ !empty($opportunity) ? (array_key_exists($field->customFields->schema, $opportunity) ? $opportunity[$field->customFields->schema] : (array_key_exists('lead', $opportunity) && array_key_exists($field->customFields->schema, $opportunity['lead']) ? $opportunity['lead'][$field->customFields->schema] : (!empty($opportunityCustomFields->toArray())&&array_key_exists($field->customFields->schema, $opportunityCustomFields->toArray())?$opportunityCustomFields[$field->customFields->schema]:''))) : ''  }}</textarea>
                                        @elseif($field->customFields->type == 'File')
                                            <input type="file" class="form-control"
                                                id="{{ $field->customFields->schema }}"
                                                accept="{{ implode(',', json_decode($field->customFields->extension, true)) }}"
                                                name="{{ $field->customFields->schema }}{{ $field->customFields->is_multiple ? '[]' : '' }}"
                                                {{ $field->customFields->is_multiple ? 'multiple' : '' }}
                                                {{ $field->customFields->mandatory ? 'required' : '' }}>
                                        @endif
