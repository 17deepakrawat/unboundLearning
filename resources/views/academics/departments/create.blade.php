...
<form id="departmentForm" action="{{ route('academics.departments') }}" method="POST">
  <div class="modal-body">
    <div class="row g-3">
      <div class="col-md-12">
        <label class="form-label" for="name">Name</label>
        <input type="text" id="name" name="name" class="form-control" placeholder="ex: Department of Engineering" autofocus />
      </div>
      ...
      <div class="col-md-12">
        ...
        <select class="form-control" name="language_ids[]" id="language_ids" multiple>
          @foreach ($languages as $language)
            <option value="{{ $language->id }}">{{ $language->name . ' (' . $language->locale . ')' }}</option>
          @endforeach
        </select>
      </div>
