    ...
    <div class="modal-body">
        <div class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <input type="text" id="name" name="name" class="form-control" placeholder="ex: My Book"
                    autofocus />
            </div>
...
<script>
    $(function() {
        ...
        $('#specialization_id').on('change', function() {
            ...
            $.ajax({
                url: "/syllabus/dropdown/scheme-by-specialization-and-vertical/" +
                    specializationId + '/' + verticalId,
                type: 'get',
                success: function(res) {
                    schemes = '<option></option>';
                    $.each(res, function(key, value) {
                        schemes += '<option value="' + value.id +
                            '"><span class="fw-medium">' +
                            value.name +
                            '</option>';
                    });
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    $.each(res, function(key, val) {
                        syllabusOption += '<option value="' + val['id'] + '">' +
                            val['name'] + '</option>';
                    });
