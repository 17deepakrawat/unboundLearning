<div class="modal-header">
    <h5 class="modal-title">Update E-Book</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
  </div>
  <form id="E-BookForm" action="{{ route('settings.lms.e-books.update',[$eBookData->id]) }}" method="POST">
    @method('PUT')
    <div class="modal-body">
      <div class="row g-3">
        <div class="col-md-4">
          ...
          <select name="vertical_id" id="vertical_id" class="form-select">
            ...
            @foreach ($verticals as $vertical)
                <option value="{{$vertical->id}}" {{$eBookData->vertical_id==$vertical->id?"selected":""}}>{{$vertical->name}}</option>                
            @endforeach
          </select>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
          <label class="form-label" for="name">E-Book Name</label>
          <input type="text" id="name" name="name" value="{{$eBookData->name}}" class="form-control" placeholder="ex: My Book" autofocus />
        </div>
      ...
      $('#vertical_id').on('change',function(){
        ...
        $.ajax({
            ...
            success:function(res)
            {
                ...
                $('#specialization_id').val({{$eBookData->specialization_id}});
      ...
      $('#specialization_id').on('change',function(){
        ...
        $.ajax({
            ...
            success:function(res)
            {
                  ...
                  $('#scheme_id').val({{$eBookData->scheme_id}});
      ...
      $('#scheme_id').on('change',function(){
            ...
            $.ajax({
                ...
                success:function(res)
                {
                    ...
                    $('#syllabus_id').val({{$eBookData->syllabus_id}});
                    $('#syllabus_id').trigger('change');
                }
            })
      });
      $("#E-BookForm").submit(function(e) {
        e.preventDefault();
        if ($("#E-BookForm").valid()) {
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              if (response.status == 'success') {
                ...
                $(".modal").modal('hide');
                $('#e-books-table').DataTable().ajax.reload();
              } else {
    ...
    $('#syllabus_id').on('change',function(){
      ...
      $.ajax({
        ...
        success:function(res)
        {
          ...
          $('#chapters_id').val({{$eBookData->chapters_id}});
    ...
    $('#chapters_id').on('change',function(){
      ...
      $.ajax({
        ...
        success:function(res)
        {
          ...
          $('#units_id').val({{$eBookData->units_id}});
    ...
    $('#units_id').on('change',function(){
      ...
      $.ajax({
        ...
        success:function(res)
        {
          ...
          $('#topics_id').val({{$eBookData->topics_id}});
        }
      })
    });
  </script>
