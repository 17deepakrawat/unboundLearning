...
@section('page-script')
  <script type="module">
    $(function() {
      var canAdd = "{{ Auth::user()->hasPermissionTo('create e-books') ? true : false }}";
      const addButton = canAdd ? {
        text: 'Add E-Book',
        className: 'add-new btn btn-primary mb-3 mb-md-0 waves-effect waves-light',
        attr: {
          'onclick': canAdd ? "add('{{ route('settings.lms.e-books.create') }}', 'modal-xl')" : "return false;",
        },
        init: function(api, node, config) {
          $(node).removeClass('btn-secondary');
        }
      } : '';
      var canEdit = "{{ Auth::user()->hasPermissionTo('edit e-books') ? true : false }}";
      var table = $('#e-books-table').DataTable({
        ajax: "{{ route('settings.lms.e-books') }}",
        columns: [{
            data: ''
          },
          {
        ...
        columnDefs: [{
          ...
          },
          {
            ...
            render: function(data, type, full, meta) {
              ...
              return '<label class="switch">' +
                '<input onclick="updateActiveStatus(&#39;/settings/lms/e-books/status/' + full['id'] + '&#39;, &#39;e-books-table&#39;)" type="checkbox" ' + $checkedStatus + ' class="switch-input">' +
          ...
          },
          {
            ...
            orderable: false,
            render: function(data, type, full, meta) {
              var edit_url = "/settings/lms/e-books/edit/"+full.id;
...
@section('content')
  ...
  <div class="card">
    <div class="card-datatable table-responsive">
      <table id="e-books-table" class="table border-top">
        <thead>
          <tr>
            <th></th>
            <th>Name</th>
