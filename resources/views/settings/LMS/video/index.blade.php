...
@section('page-script')
  <script type="module">
    $(function() {
      var canAdd = "{{ Auth::user()->hasPermissionTo('create videos') ? true : false }}";
      const addButton = canAdd ? {
        text: 'Add Video',
        className: 'add-new btn btn-primary mb-3 mb-md-0 waves-effect waves-light',
        attr: {
          'onclick': canAdd ? "add('{{ route('settings.lms.video.create') }}', 'modal-xl')" : "return false;",
        },
        init: function(api, node, config) {
          $(node).removeClass('btn-secondary');
        }
      } : '';
      var canEdit = "{{ Auth::user()->hasPermissionTo('edit videos') ? true : false }}";
      var table = $('#video-table').DataTable({
        ajax: "{{ route('settings.lms.videos') }}",
        columns: [{
            data: ''
          },
          {
        ...
        columnDefs: [{
          ...
          },
          {
            ...
            render: function(data, type, full, meta) {
              ...
              return '<label class="switch">' +
                '<input onclick="updateActiveStatus(&#39;/settings/lms/video/status/' + full['id'] + '&#39;, &#39;video-table&#39;)" type="checkbox" ' + $checkedStatus + ' class="switch-input">' +
          ...
          },
          {
            ...
            orderable: false,
            render: function(data, type, full, meta) {
              var edit_url = "/settings/lms/video/edit/"+full.id;
...
@section('content')
  ...
  <div class="card">
    <div class="card-datatable table-responsive">
      <table id="video-table" class="table border-top">
        <thead>
          <tr>
            <th></th>
            <th>Name</th>
