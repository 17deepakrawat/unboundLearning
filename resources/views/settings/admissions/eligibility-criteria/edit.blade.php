<div class="modal-header">
    <h5 class="modal-title">Update Eligibility Criterion</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
  </div>
  <form id="addEligibilityCriterionForm" action="{{ route('settings.admissions.eligibility-criteria.update', [$EligibilityCriterion->id]) }}" method="POST">
    <div class="modal-body">
      <div class="row g-3">
        <div class="col-md-12">
          <label class="form-label" for="name">Name</label>
          <input type="text" id="name" name="name" value="{{ $EligibilityCriterion->name }}" class="form-control" placeholder="ex: Graduation" autofocus />
        </div>
        <div class="col-md-12">
          <label class="form-label" for="vertical_ids">Vertical <small class="text-muted">(Multiple)</small></label>
          <select class="form-control" name="vertical_ids[]" id="vertical_ids">
