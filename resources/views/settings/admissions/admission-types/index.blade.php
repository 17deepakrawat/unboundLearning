...
@section('page-script')
  <script type="module">
    $(function() {
      var canAdd = "{{ Auth::user()->hasPermissionTo('create admission-types') ? true : false }}";
      const addButton = canAdd ? {
        text: 'Add Admission Type',
        className: 'add-new btn btn-primary mb-3 mb-md-0 waves-effect waves-light',
        attr: {
          'onclick': canAdd ? "add('{{ route('settings.admissions.admission-types.create') }}', 'modal-md')" : "return false;",
        },
        init: function(api, node, config) {
          $(node).removeClass('btn-secondary');
        }
      } : '';
      var canEdit = "{{ Auth::user()->hasPermissionTo('edit admission-types') ? true : false }}";
      var dataTableAdmissionTypes = $('#admission-types-table').DataTable({
        ajax: "{{ route('settings.admissions.admission-types') }}",
        columns: [{
            data: ''
          },
          {
        ...
        columnDefs: [{
          ...
          },
          {
            ...
            render: function(data, type, full, meta) {
              ...
              var $isDisabled = canEdit ? 'onclick="updateActiveStatus(&#39;/settings/admissions/admission-types/status/' + full['id'] + '&#39;, &#39;admission-types-table&#39;)"' : 'onclick="return false;"';
...
@section('content')
  <h4 class="mb-4">Admission Types</h4>

  <div class="card">
    <div class="card-datatable table-responsive">
      <table id="admission-types-table" class="table border-top">
        <thead>
          <tr>
            <th></th>
            <th>Name</th>
