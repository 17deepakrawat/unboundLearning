...
{{-- {{ route('manage.leads.re-assign.update', [$opportunity->id]) }} --}}
<form action="/manage/leads/re-assign-multiple/update" id="reAssignForm" method="POST">
  <div class="modal-body">
    <div class="row g-3">
      ...
      <div class="col-md-12">
        <h5><span class="fs-18">{{ $leadCount }}</span> Leads will re-assign!</h5>
      </div>
    </div>
  </div>
...
<script>
  ...
  $("#reAssignForm").submit(function(e) {
    ...
    if ($("#reAssignForm").valid()) {
      ...
      formData.append("leadIds", "{{ implode(",", $ids) }}");
      $.ajax({
