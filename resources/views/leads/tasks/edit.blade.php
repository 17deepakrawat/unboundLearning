...
<form id="taskUpdateForm" action="{{ route('manage.opportunity.task.update') }}" method="POST">
      ...
      <div class="col-md-12">
        ...
        <select class="form-control" id="stage_id" name="stage_id" onchange="getSubStages()">
          ...
          @foreach ($stages as $stage)
            <option value="{{ $stage->id }}" {{ $stage->id == $task->opportunity->stage_id ? 'selected' : '' }}>{{ $stage->name }}</option>
          @endforeach
        </select>
      </div>
      <div class="col-md-12">
...
<script>
  ...
  function getSubStages() {
    ...
    const selectedSubStageId = "{{ $task->opportunity->sub_stage_id }}";
    $.ajax({
