...
function validateFormData(fieldsets) {
    ...
    $(fieldsetid + ".fileext").each(function () {
        ...
        if (file != NaN || file != undefined) {
            ...
            if ($.inArray(ext, res) == -1) {
                var message = "The Selected File should be in " + ext + " format.";
                if (!$('#' + id + '-error-message').length) {
    ...
    $(fieldsetid + ".checkFileSize").each(function () {
        ...
        if (file != NaN || file != undefined) {
            ...
            if (parseFloat(fileSizeInKb) > parseFloat(limitsize)) {
                var message = "The Selected File Size Can't Be More Than " + fileSizeInKb + " KB."
...
function submitStepForm(formid,isfinal)
{
    ...
    if(isfinal=='final')
    {
        formData.append('step_status','complete');
    }
    else
    {
        formData.append('step_status',formid);
    }
    if(localStorage.getItem('app_id') !==null)
    {
        var app_id = localStorage.getItem('app_id');
        var url = '/manage/student/application/update/'+app_id;
    }
    else
    {
       var url = '/manage/student/application/store';
    }
    $.ajax({
        url: url,
        type:"POST",
