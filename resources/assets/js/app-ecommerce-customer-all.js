...
$(function () {
  ...
  if (dt_customer_table.length) {
    var dt_customer = dt_customer_table.DataTable({
      ajax: '/manage/leads/list', // JSON file to add data
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $name = full['lead']['first_name'],
              $email = full['lead']['email'],
              $image = full['lead']['avatar'];
        ...
        },
        {
          ...
          targets: 4,
          render: function (data, type, full, meta) {
            var $vertical = full['vertical'] ? full['vertical']['short_name'] : '';
            var $program = full['program'] ? full['program']['name'] : '';
            var $specialization = full['specialization'] ? full['specialization']['name'] : '';
            return $vertical + '<br> ' + $program + '<br>' + $specialization;
          }
        },
        {
          // customer Status
          targets: 5,
          render: function (data, type, full, meta) {
            var $source = full['lead']['source']['name'];
            var $subSource = full['lead']['subSource'] ? full['lead']['subSource']['name'] : '';
            return $source + '<br>' + $subSource;
          }
        },
        {
          // customer Spent
      ...
      buttons: [
        ...
        {
          text: '<i class="ti ti-plus me-0 me-sm-1 mb-1 ti-xs"></i><span class="d-none d-sm-inline-block">Add Lead</span>',
